...
export async function GET() {
  try {
    ...
    const [{ data: awardees, error: awardeeError }, { data: period, error: periodError }] = await Promise.all([
      admin
        .from("awardees")
        .select("id,full_name,campus,major,cohort,region,photo_url")
        .eq("status", "active")
        .order("full_name", { ascending: true }),
      admin
        .from("assessment_periods")
        .select("id,name,academic_year,semester,starts_at,ends_at")
        .eq("is_active", true)
    ...
    return NextResponse.json({ awardees: awardees ?? [], period: period ?? null }, { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
